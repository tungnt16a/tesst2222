...
public class OAuthUtils {
	...
	public static OAuth2Details createOAuthDetails(Properties config) {
		...
		oauthDetails.setCode((String) config.get(OAuthConstants.CODE));
		oauthDetails.setRedirectUri((String) config.get(OAuthConstants.RE_DIRECT_URI));
	...
	public static String getAccessToken(OAuth2Details oauthDetails) {
		...
		String code = oauthDetails.getCode();
		String redirectUri = oauthDetails.getRedirectUri();
		...
		parametersBody.add(new BasicNameValuePair(
					OAuthConstants.RE_DIRECT_URI, redirectUri));
		...
		try {
			...
			post.setHeader("Authorization", "Basic ZDA5NDczYTAtMGE5Mi00OTQ1LWFhMWItZDM1MzllNDgyOGJlOmNkOTUxZTA5LWY2YmQtNDU1ZS05OTc4LTllMGZiYWI0ODUzNA==");

			response = client.execute(post);
			int status = response.getStatusLine().getStatusCode();
			if (status == OAuthConstants.HTTP_UNAUTHORIZED) {
				System.out
